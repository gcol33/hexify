...
```{r point-sizes, fig.width=7, fig.height=10}
par(mfrow = c(3, 2))
...
For ggplot2 users, `hexify_ggplot()` returns a ggplot object that can be further customized.
...
Since `hexify_ggplot()` returns a ggplot object, you can add layers and modify themes:
...
For choropleth-style visualizations with aggregated data, use `hexify_heatmap()`.
...
```{r heatmap-palettes, fig.width=8, fig.height=8}
...
| `hexify_ggplot()` | ggplot2 plotting, returns modifiable ggplot object |
| `hexify_heatmap()` | Choropleth heatmap with color scale |
