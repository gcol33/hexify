...
parent_cell <- hexify_index_to_cell(parent_idx, 7, "z7")
cat(sprintf("  Parent index: %s (cell %d)\n", parent_idx, parent_cell))
```
...
cat("------  --------------  -------------------\n")
for (h3_res in 0:8) {
  g <- hex_grid(resolution = h3_res, type = "h3")
  h3_area <- hexify_eff_res_to_area(h3_res, "h3")
  isea_res <- dg_closest_res_to_area(h3_area, aperture = 7)
  cat(sprintf("   %2d   %14.1f  res %d (%.1f km²)\n",
              h3_res, h3_area, isea_res,
              hexify_eff_res_to_area(isea_res, aperture = 7)))
}
```
