...
**`HexGrid`** — A grid specification that stores all parameters (aperture, resolution, area). Define once, reuse everywhere.
...
The key workflow: create a `HexGrid` once, then apply it to multiple datasets.
...
Given a data frame with longitude/latitude columns, `hexify()` returns a `HexData` object containing your original data plus:
...
|----------|-------------|
| `hex_grid()` | Create a HexGrid specification |
