...
```{r cover-image, echo=FALSE, message=FALSE, warning=FALSE, fig.width=7, fig.height=3.5, fig.align='center'}
...
The sf package holds spatial data as simple features: a data frame with a
geometry column of points, lines or polygons, and a CRS that says how to read
their coordinates. hexify takes an sf object wherever it takes a data frame,
and transforms the coordinates itself when the CRS differs from the grid's.
...
```{r bird-plot, fig.width=7, fig.height=5.25, message=FALSE, warning=FALSE}
...
```{r richness-plot, fig.width=7, fig.height=5.25, message=FALSE}
ggplot() +
  geom_sf(data = region, fill = "gray95", color = "gray70", linewidth = 0.2) +
