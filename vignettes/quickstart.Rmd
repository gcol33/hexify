...
The simplest approach uses the built-in `plot()` method (basemap is shown by default):
...
Points are automatically jittered within their hexagon cells, with size
scaled based on density. The presets control how much of the densest
hexagon is covered by points:

- `"tiny"` (~10% coverage)
- `"small"` (~25% coverage)
- `"normal"` / `"auto"` (~50% coverage)
- `"large"` (~70% coverage)
- `"very large"` (~90% coverage)
...
```{r plot-points-sized, fig.width=7, fig.height=5}
