...
```{r lambert-geometry, echo=FALSE, fig.width=8, fig.height=4}
# Lambert azimuthal equal-area: show the projection concept
par(mfrow = c(1, 2), mar = c(1, 1, 3, 1), bg = "white")

# Left: Cross-section showing angular distance
plot(NULL, xlim = c(-1.5, 1.5), ylim = c(-0.3, 2.3), asp = 1,
     axes = FALSE, xlab = "", ylab = "",
     main = "Angular distance on sphere")
...
# Tangent point S at bottom
points(0, 0, pch = 19, cex = 1.5, col = "black")
text(0, -0.2, "S (center)", cex = 0.9)

# Center of sphere
points(0, R, pch = 3, cex = 1.2, col = "gray50")
text(0.15, R + 0.12, "O", cex = 0.9, col = "gray50")

# Point P at angular distance c = 60°
c_ang <- 60 * pi/180
P_x <- R * sin(c_ang)
P_y <- R - R * cos(c_ang)

points(P_x, P_y, pch = 19, cex = 1.3, col = "#E63946")
text(P_x + 0.15, P_y + 0.1, "P", cex = 1.1, col = "#E63946", font = 2)

# Draw arc showing angular distance c (along sphere surface from S to P)
arc_angles <- seq(-pi/2, -pi/2 + c_ang, length.out = 30)
lines(R * cos(arc_angles), R + R * sin(arc_angles), lwd = 2.5, col = "#E63946")

# Label the angular distance
text(0.55, 0.35, expression(italic(c)), cex = 1.2, col = "#E63946")

# Draw radius lines to show the angle
lines(c(0, 0), c(R, 0), lty = 2, col = "gray60")
lines(c(0, P_x), c(R, P_y), lty = 2, col = "gray60")

# Right: The projection formula
plot(NULL, xlim = c(-1.8, 1.8), ylim = c(-1.8, 1.8), asp = 1,
     axes = FALSE, xlab = "", ylab = "",
     main = "Projected plane")

# Draw the projected plane as a disk
theta_seq <- seq(0, 2*pi, length.out = 100)
max_rho <- 2  # maximum extent
lines(max_rho * cos(theta_seq), max_rho * sin(theta_seq),
      lwd = 1, col = "gray70", lty = 2)

# Origin (projection of S)
points(0, 0, pch = 19, cex = 1.5, col = "black")
text(0.15, -0.15, "S'", cex = 0.9)

# Project P: rho = 2R*sin(c/2)
rho_P <- 2 * R * sin(c_ang / 2)
# Place at same azimuth (let's say 30° from vertical for visibility)
az <- 30 * pi/180
P_prime_x <- rho_P * sin(az)
P_prime_y <- rho_P * cos(az)

points(P_prime_x, P_prime_y, pch = 19, cex = 1.3, col = "#457B9D")
text(P_prime_x + 0.15, P_prime_y + 0.1, "P'", cex = 1.1, col = "#457B9D", font = 2)

# Draw line from origin to P'
lines(c(0, P_prime_x), c(0, P_prime_y), lwd = 2, col = "#457B9D")

# Label rho
text(P_prime_x/2 - 0.15, P_prime_y/2 + 0.1, expression(rho), cex = 1.1, col = "#457B9D")

# Add concentric circles showing equal-area bands
for (c_val in c(30, 60, 90) * pi/180) {
  rho_val <- 2 * R * sin(c_val / 2)
  lines(rho_val * cos(theta_seq), rho_val * sin(theta_seq),
        lwd = 1, col = "gray80")
}

# Formula annotation
text(0, -1.5, expression(rho == 2*R*sin(frac(c,2))), cex = 1.1)
```
