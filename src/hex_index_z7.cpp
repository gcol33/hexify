// hex_index_z7.cpp
// Z7 Encoding/Decoding Implementation
// EXACT port of DGGRID's DgZ7StringRF.cpp encoding logic
// With added canonical form support
...
std::string encode(int quadNum, long long i, long long j, int resolution) {
    ...
    // Allocate (res + 1) elements - this matches DGGRID exactly
    ...
    // EXACT DGGRID LOGIC from DgZ7StringRF.cpp line 228
    // Note: This uses the REMAPPED baseCell, which is the bug in DGGRID
    // but we're reproducing it exactly for compatibility
    IVec3D::Direction skipDigit = ((baseCell < 6) ? 
        (IVec3D::Direction)IVec3D::PENTAGON_SKIPPED_DIGIT_TYPE1 : 
        (IVec3D::Direction)IVec3D::PENTAGON_SKIPPED_DIGIT_TYPE2);
    
    // EXACT DGGRID LOGIC from lines 229-243
    bool skipRotate = false;
    bool firstNonZero = false;
    
    for (int r = 1; r < res + 1; r++) {  // Note: r goes to res+1 like DGGRID
        ...
        // DGGRID rotates ALL digits when skipRotate is true
        // (including the first non-zero digit!)
        if (skipRotate) {
