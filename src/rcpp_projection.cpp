...
NumericVector cpp_snyder_forward(double lon_deg, double lat_deg) {
  auto out = hexify::snyder_fwd(lon_deg, lat_deg);
...
NumericVector cpp_project_to_tx_ty(int face, double lon_deg, double lat_deg) {
  auto xy = hexify::snyder_fwd_face(face, lon_deg, lat_deg);
  return NumericVector::create(_["tx"] = xy.first,
                               _["ty"] = xy.second);
}
...
Rcpp::NumericVector cpp_icosa_face_params(int face) {
  ...
  return Rcpp::NumericVector::create(
    ...
    _["dazh"]    = hexify::snyder_face_dazh_rad(face)
  );
}

// [[Rcpp::export]]
Rcpp::NumericVector cpp_hex_index_face_to_lonlat(double x, double y,
                                                 double cen_lat, double cen_lon, double dazh,
                                                 bool degrees = true,
                                                 Rcpp::Nullable<double> tol = R_NilValue,
                                                 Rcpp::Nullable<int>    max_iters = R_NilValue) {
