...
#' Construct a hexagonal grid specification
#' 
...
#' fine_grid <- hexify_construct(area = 100, aperture = 3, resround = "up")
#' }
...
#' grid integrity.
#' 
#' @param dggs Grid object to verify (from hexify_construct or dgconstruct)
...
dgverify <- function(dggs) {
  ...
  if (!inherits(dggs, "hexify_grid") && !inherits(dggs, "dggs")) {
    stop("dggs must be a grid object from hexify_construct() or dgconstruct()")
  }
