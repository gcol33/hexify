...
hexify <- function(data,
  ...
  if (is_mixed_aperture) {
    ...
    seqnum <- cpp_lonlat_to_seqnum_dggrid_ap43(lon_vec, lat_vec,
                                                grid$resolution, mixed_aperture_level)

    # Get cell centers
    centers <- cpp_seqnum_to_lonlat_dggrid_ap43(seqnum, grid$resolution, mixed_aperture_level)
  ...
  EARTH_SURFACE_KM2 <- 510065623  # Earth surface area in km²
